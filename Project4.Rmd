...
qplot(x = chlorides, data = wines, binwidth = 0.005, xlim = c(0, 0.2))
```
...
No operations were needed to tidy the data, all observations were complete. There were some unusual distributions and outliers. The outliers seem to be true data values rather than input errors, though, so I did not want to get rid of any of these values.
...
```{r Bivariate_Plots, fig.height=12, fig.width=12}
features = c('fix acid', 'vol acid', 'cit acid', 'sugar', 'chlor', 'free SO2', 'total SO2', 'density', 'pH', 'sulphates', 'alc', 'quality')
...
```{r, fig.width=12}
q1 = qplot(data = wines, x = total.sulfur.dioxide, fill = as.factor(quality), position = 'fill')
q2 = qplot(data = wines, x = free.sulfur.dioxide, fill = as.factor(quality), position = 'fill')
...
```{r}
qplot(data = wines, x = alcohol, fill = as.factor(quality), position = 'fill')
qplot(data = wines, x = volatile.acidity, fill = as.factor(quality), position = 'fill')
```
...
```{r}
qplot(data = wines, x = alcohol, fill = as.factor(quality), binwidth = 0.1, position = 'fill')
```
